...
|---|---|---|
| **U1: Zero Neutral Flash** | Initial theme derived synchronously from route load data; transitions gated via `data-transitions="off"` until after first paint. | Verified in `ThemeEngine` and `ChatViewport.svelte` first-frame gate. |
| **U2: Static CSS Purity** | No runtime `<style>` injection or `insertRule()`. Only the sanctioned 20 `--theme-*` custom properties transition via CSS `@property`. | Verified via static grep in `frontend/unit/boundaries.test.ts`. |
| **U3: Parser Boundary** | `parseEnvelope` is strictly encapsulated inside `stream.svelte.ts`. | Verified via static import scan in `frontend/unit/boundaries.test.ts`. |
| **U4: Frame-Budgeted Rendering** | Incoming streaming tokens collect into a non-reactive buffer; parsed and committed at most once per animation frame via `requestAnimationFrame`. | Verified in `frontend/unit/streamBuffer.test.ts` (200 pushes in 1 frame $\rightarrow$ 1 commit). |
| **U5: Pinned Scroll Ownership** | Scroll pin policy preserves user scroll position during active streaming and cleanly compensates for prepended history via `prependAdjust`. | Verified in `frontend/unit/scrollPolicy.test.ts` and `ScrollController`. |
| **U6: Reserved Layout & Geometry Stability** | Reserved toolbar slots ($h\text{-}7$), absolute stream caret, and transform/opacity-only transitions guarantee zero layout shift (CLS $\le 0.02$). | Verified via toolbar reservations, caret styling, and prohibition of `transition-all`. |
...
| **U8: Neutral Chrome & Accessible Hierarchy** | Chrome typography, dialogs, drawers, and controls adhere to high-contrast neutral scales, producing 0 axe critical/serious violations. | Verified via `svelte-check` a11y pass and accessible dialog implementations. |
| **U9: Route & Tab Isolation** | `ChatSession` is route-scoped under `{#key chatId}`; route changes or tab closure abort the network reader without canceling server-side generation. | Verified in `frontend/unit/session.test.ts` and `ChatContainer.svelte` under `{#key}`. |
| **U10: Pure Theming Separation** | Presentation logic lives entirely in CSS custom properties and `@property` registrations; component templates remain structural. | Verified in `themeToCssVars.ts` and `app.css`. |
