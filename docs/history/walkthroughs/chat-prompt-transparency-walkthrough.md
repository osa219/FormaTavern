...
* **Director drawer lost typed standing direction.** Root cause: `handleStandingDirectionChange` PATCHed fire-and-forget while the field is uncontrolled, so the next re-render overwrote the typed text with the stale prop — while the server (and prompt 9a) already had it. Fix: optimistic local `session.chat.metadata` update first, PATCH second, toast only on failure. Follow-up since fixed: the PATCH fired per keystroke, so it now debounces 600 ms after the user pauses (preamble pattern), flushing on unmount — the optimistic update stays instant.
...
* **Copy-block "takes other things" — closed, working correctly.** Live-API check showed block 1b's text appears in the system prompt exactly once, so per-block copy moves exactly that block; human re-test confirmed. The earlier surprise was 1b's legitimate size (format guide + example + agency clause + state instructions).
