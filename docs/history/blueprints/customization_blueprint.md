...
| **A-U1b** | Shell accent has one resolved value with explicit precedence `character > shell.colors.accent > shell.chrome.accent > neutral`. `shellToThemeOverrides` resolves `accent = shell.colors?.accent ?? shell.chrome?.accent`; `ShellSurface` uses the same `resolvedAccent` for its inline `--theme-accent`. `chrome.accent` is retained as a legacy alias, not a second source. | Post-implementation fix: the dual-source spread silently let `colors` win over `chrome`. Documented fallback keeps existing shell docs working while giving the cascade a single predictable value. |
| **A-SURF1** | `+layout.svelte` never sets `data-ft-surface` on `<html>` and never writes `--theme-*`/`--chrome-*` to `documentElement`. It sets only `data-ft-motion`, `data-ft-chrome`, and the C1-clean mirror `data-ft-active-surface` (sourced from `SURFACES`). All theme/chrome vars live on surface roots (`ShellSurface` inline vars, `themeEngine.styleAttr`). `app.css` provides only the generic `[data-ft-surface]` chrome-bg/surface/line derivation plus shell-only color/bg. The Studio `LivePreview` character mock (own outlet + `previewChromeReset`) is the documented exemption to C13 non-nesting: Studio intentionally hosts 2 sheets (shell + preview), isolated by design. | Post-implementation fix: html-level surface + vars duplicated surface state, forced a set-then-reset cycle in `app.css`, and made §2.1 "surfaces never nest" false. Enforced by `frontend/unit/surfaces.test.ts` (no `data-ft-surface` on html/layout, allowlist of surface roots). |
