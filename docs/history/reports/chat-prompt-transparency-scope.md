# Chat Frontend + Prompt Transparency Scope (Draft for Discussion)
...
* Optimistic turn builds `segments` locally — `frontend/src/lib/state/session.svelte.ts:135-155` — so it renders briefly.
* `POST /chats/:id/messages` inserts the user row without `segments` — `backend/src/routes/chats.ts:351-362` — defaulting to `[]` in `backend/src/db/repositories/messages.ts:43`.
* `done` triggers `refetchState()` which replaces the list with the server version where the user turn has zero segments.
* `MessageTurn.svelte` renders `segments` only, so the turn collapses to an empty toolbar slot.

**Agreed fix direction:**
* Backend stores a single segment for user rows (`persona` / `npc` / `narrator` / `character` per `narrativeRole` + `senderName`), matching the optimistic shape.
* Frontend falls back to rendering `content` when `segments` is empty, so old rows self-heal.
* Investigate `[state]` leak separately: model emits `[state]` plain text (not the ```state fence), parser leaves it as prose. See §3 for the instruction-side fix; display-side must never render state as bubble text.
...
**Agreed fix direction:**
* Neutralize the example: no concrete `Guide`, or omit NPC lines when `chat.metadata.npcs` is empty; use `{{char}}` only.
* Add an explicit state-block syntax example in the matching dialect.
* Surface an empty-card warning (Studio + chat) when blocks 2–5 contribute nothing — reply will be mostly template.
...
6. Priority order for implementation (proposed: §2 → §3 → §7 → §5 → §4 → §6, pending chat-page bug list from the reporter).
