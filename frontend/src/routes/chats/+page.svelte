...
<ShellSurface>
  ...
  <header class="sticky top-0 z-30 flex h-14 w-full items-center justify-between border-b border-(--chrome-line) chrome-bar px-4 md:px-6">
    <div class="flex min-w-0 items-center gap-2 md:gap-3">
      <a
        href="/"
        class="flex min-w-0 items-center gap-2 font-bold tracking-wide text-(--chrome-text) uppercase hover:text-accent transition-colors shrink-0"
      >
        <span class="text-accent text-lg">◈</span>
        <span class="text-sm truncate">{shellTheme.theme.labels?.foyerTitle || 'FormaTavern'}</span>
      </a>
      <span class="text-(--chrome-text)/40 shrink-0">/</span>
      <span class="rounded border border-(--chrome-line) bg-(--chrome-surface) px-2 py-0.5 text-[10px] font-mono text-(--chrome-text)/70 shrink-0">
        Chats Hub
      </span>
    </div>
